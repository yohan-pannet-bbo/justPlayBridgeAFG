...
function BBO_SetTableOptions() {
  ...
  BBO_constructAndSendXMLToTable("jt_set_url ", ["which", "url", "cookies"],  ["play", "https://arcade-proxy.herokuapp.com/v1/gib", "n"]);
  BBO_constructAndSendXMLToTable("jt_set_url ", ["which", "url"],             ["explain", "https://arcade-proxy.herokuapp.com/v1/explain"]);
  BBO_constructAndSendXMLToTable("jt_set_url ", ["which", "url", "cookies"],  ["dealer", "https://arcade-proxy.herokuapp.com/v1/deal", "n"]);
  //BBO_constructAndSendXMLToTable("jt_set_url ", ["which", "url", "cookies"], ["play", "//gibrest.bridgebase.com/u_bm/robot.php?sc=tp", "y"]);
  //BBO_constructAndSendXMLToTable("jt_set_url ", ["which", "url"], ["explain", "//gibrest.bridgebase.com/u_bm/u_bm.php"]);
...
function BBO_SetupPlayerNameDiv(cb) {
  ...
  $('#playercount').empty();
  $("#playercount").append(BBOArcade['catalog'].AR004 + ': <span id="numberofplayers"></span>');
...
function BBO_LeaderboardUpdate(lb) {
  ...
  lb.forEach(function (currentValue, index, fullArray) {
    ...
    if (!(index % numCols)) {
      ...
      lineText = counter + '. ' + fullArray[index] + ' ' + fullArray[index + 1];
      counter++;
